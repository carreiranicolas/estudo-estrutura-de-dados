...
\begin{document}
	...
	Para declarar uma constante existem duas formas. Na primeira, devemos  utilizar \#define nome-costante <valor> no começo do programa, ANTES DO INICIO DO CÓDIGO. Uma detalhe é que neste caso, não usaremos ponto e virgula no final. Veja:
	...
	O o switch, como podemos ver acima é próprio para testar uma variavel em relação a diversos valores pré-estabelecidos. Além disso, como podemos ver acima o default irá desempenhar o valor que o else desempenha na estrutura if-else. Veja um exemplo:
	
		
	\begin{center}
		\includegraphics[width=7cm]{imagens/switch-case.png}
	\end{center}
	...
	Significa basicamente "enquanto". Enquanto a condição for verdadeira, ele excutará o loop. Veja como é a sintaxe:
	...
	\begin{itemize}
		\item Inicializacao: iniciar variáveis (contador).
		\item Condicao: avalia a condição. Se verdadeiro, executa comandos do bloco, senão encerra laço.
		\item Incremento: ao término do bloco de comandos, incrementa o valor do contado
	\end{itemize}
	
	Perceba então que o loop for executa um número controlado de vezes, a partir do seu contador que passamos na sua iniciliazação
...
int main(void) {
	...
	\begin{itemize}
		\item A altura (h) de uma árvore binária é a longitude do maior caminho. Ou o números de vezes que se aplica a relação pai/filho no maior caminho( igual ao número de ramos/galhos). 
		\item O nível de um elemento dentro da árvore binária se define como a longitude do caminho que parte da raiz e chega até esse elemento. Desta forma, o nível da raiz é 0, e o nível de qualquer
		elemento é o nível de seu pai mais 1.
	...
	Por outro lado, se a árvore estiver degenerada (os nós internos tem somente uma subarvore), usaremos a fórmula:
	...
	Uma curiosidade é que tem algumas nomenclaturas que podemos dar para as árvores a depender do seu grau de semelhança com outras arvores:
	
	\begin{itemize}
		\item Arvores iguais: Duas árvores binarias são iguais se ambas são vazias, ou se suas raízes são iguais, e o mesmo acontece para suas respectivas subarvores
		esquerda e direita.
		\item Arvores isomorfas: Duas árvores são isomorfas se têm a mesma forma(estrutura), mas não necessariamente os mesmos elementos
		\item Arvores semelhantes: Duas árvores são semelhantes se têm os mesmos elementos, ainda que não sejam isomorfas.
	\end{itemize}
	
	\subsubsection{Percurso}
	...
	\begin{itemize}
		\item Percurso pré-ordem: Visita a raiz, o filho da esquerda e o filho da direita
		\item Percurso um-ordem: Visita o filho da esquerda, a raiz e o filho da direita
		\item Percurso pós-ordem: Visita o filho da esquerda, o filho da direita e a raiz
	\end{itemize}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_PO.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_UO.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_POO.png}
		
	\end{center}
	
	
	
	\section{Arvore binaria de busca}
	
	Uma arvore binaria de busca é uma árvore binária com uma regra fundamental. 
	
	\begin{itemize}
		
		\item tudo na subarvore à esquerda é menor que ele
		\item tudo na subarvore à direita é maior que ele
	\end{itemize}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_busca.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_busca_insercao.png}
		
	\end{center}
	
	\subsubsection{Remoção}
	
	Remover um nó de uma árvore binária de busca não é uma tarefa tão simples quanto a inserção. Isso ocorre porque precisamos procurar o nó a ser removido da
	árvore o qual pode ser um nó folha ou um nó interno e se for um nó interno, precisamos reorganizar a árvore para que ela continue sendo uma árvore binária de busca
	
	Veja um exemplo de remoção de um NÓ FOLHA:
	
	\begin{center}
		
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_busca_remocao1.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_busca_remocao2.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvore_binaria_busca_remocao3.png}
	...
	Infelizmente, os algoritmos de inserção e remoção em árvores binárias não garantem que a árvore gerada a cada passo esteja balanceada. Dependendo da ordem em que os dados são inseridos na árvore,
	podemos criar uma árvore na forma de uma escada. Veja a imagem abaixo:
	...
	Assim, surgiram as árvores balanceadas, sendo elas:
	...
	Assim, nós teremos as rotações que buscarão corrigir o fator de balanceamento (ou fb), que é a diferença entre a altura da subarvore à esquerda - a altura da subarvore à direita. Veja a imagem abaixo:
	
	\begin{center}
		
		\includegraphics[width=10cm,height=7cm,keepaspectratio=false]{imagens/arvores_avl_fb.png}
		
	\end{center}
	
	Acima, perceba que o fator de balanceamento é igual a 2. Isso acontece porque a altura da árvore da esquerda é 3 e a altura da árvore à direita é 1, ou seja 3 - 1 = 2. Dessa forma, perceba que precisamos balancear a árvore, uma vez que o fator de balanceamento deve ser 1, 0 ou -1. Para isso, vamos aplicar as rotações
	...
	\begin{itemize}
		\item Rotação simples: 	O nó desbalanceado (pai), seu filho e o seu neto estão todos no mesmo sentido de inclinação. Existe a rotação simples à direita (LL) e rotação dupla à esquerda (RR)
		\item Rotação dupla: O nó desbalanceado (pai) e seu filho estão inclinados no sentido inverso ao neto (equivale a duas rotações simples). Existe a rotação dupla à direita (LR) e rotação dupla à esquerda (RL)
	\end{itemize}
	
	\subsubsection{Rotação LL (simples à direita)}
	Considerando um exemplo onde A é o nó desbalanceado, o que acontece é que um novo nó é inserido na sub-árvore da esquerda do filho esquerdo de A
	
	Ou seja, é necessário fazer uma rotação à direita, de modo que o nó intermediário B
	ocupe o lugar de A, e A se torne a sub-árvore direita de B. Veja abaixo:
	
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_ll.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_ll2.png}
	...
	\begin{center}
		...
		\includegraphics[width=6cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_ll3.png}
		
	\end{center}
	
	
	
	\subsubsection{Rotação RR (simples à esquerda)}
	...
	Ou seja, é necessário fazer uma rotação à esquerda, de modo que o nó intermediário B
    ocupe o lugar de A, e A se torne a sub-árvore esquerda de B. Veja abaixo:
    
    			
    \begin{center}
    	
    	\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rr.png}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rr2.png}
	...
	\begin{center}
		...
		\includegraphics[width=6cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rr3.png}
		
	\end{center}
	
	\subsubsection{Rotação LR (rotação dupla à direita)}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_lr.png}
		
	\end{center}
	
	Arvore depois de aplicar a rotação RR no nó B:
	
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_lr2.png}
		
	\end{center}
	
	
	Arvore depois de aplicar a rotação LL no nó A e por consequencia balanceando a árvore:
	
		
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_lr3.png}
		
	\end{center}
	
	\subsubsection{Rotação RL}
	...
	\begin{center}
		...
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rl.png}
		
	\end{center}
	
	Arvore depois de aplicar a rotação LL no nó C:
	
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rl2.png}
		
	\end{center}
	
	Arvore depois de aplicar a rotação RR no nó A e por consequencia balanceando a árvore:
	
		
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_rl3.png}
		
	\end{center}
	
	\subsubsection{Macete para as rotações}
	
			
	\begin{center}
		
		\includegraphics[width=10cm,height=6cm,keepaspectratio=false]{imagens/arvores_avl_macete_rotacoes.png}
	...
	\begin{itemize}
		\item Precisamos olhar item por item. Isso pode ser muito lento e esse é um processo O(n)
	\end{itemize}
	
	Outro detalhe é que as árvores binárias de busca até resolvem esse problema, mas de forma parcial, uma vez que:
	
	\begin{itemize}
		\item Podem ficar desequilibradas
		\item Podem perder eficiência
		\item Mesmo no melhor caso, fazem buscas em O(log n)
	\end{itemize}
	
	Dessa forma, faltava uma estrutura que permitia
	
	\begin{itemize}
		\item buscar muito rápido, sem percorrer tudo
		\item em tempo constante, O(1)
		\item sem depender de ordenação
		\item sem depender de estrutura balanceada
	\end{itemize}
	
	E foi assim que surgiu a tabela hash, uma estrutura extremamente eficiente para buscas, inserçoes e remoções
	...
	\begin{itemize}
		\item recebe a chave
		\item aplica algum cálculo
		\item devolve um número
		\item esse número é usado como posição no array
	\end{itemize}
	...
	\begin{center}
		...
		\includegraphics[width=12cm,height=7cm,keepaspectratio=false]{imagens/tabela_hash.png}
	...
	\begin{center}
		...
		\includegraphics[width=8cm,height=6cm,keepaspectratio=false]{imagens/tabela_hash_colisao.png}
		
	\end{center}
	
	Isso pode acontecer porque a tabela hash tem tamanho limitado, mas você tem infinitas chaves possiveis
